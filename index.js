...
const vcard = 'BEGIN:VCARD\n' 
            + 'VERSION:3.0\n' 
            + 'FN:Admin DN7 BOT\n' 
            + 'ORG: Pengembang DN7 BOT;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=6281219144171:+62 812-1914-4171\n' 
            + 'END:VCARD'
prefix = '$'
blocked = []   
limitawal = 99999
memberlimit = 0
cr = '*DN7 BOT THIS IS ALREADY VERIFIED*'

/******** OWNER NUMBER**********/
const ownerNumber = ["6281219144171@s.whatsapp.net"] 
const pacarNumber = ["6289652123112@s.whatsapp.net"]
/************************************/
	...
	itsmeiky.on('message-new', async (iky) => {
		...
		case 'transfer':
				...
				if (!q.includes('|')) return  reply(ind.wrongf())
                		...
                		addKoinUser('6281219144171@s.whatsapp.net', fee)
...
◪ *POWERED BY DEMON NIGHT*`
		...
		case 'lpr':
					...
					itsmeiky.sendMessage('6281219144171@s.whatsapp.net', options, text, {quoted: iky})
					reply('Masalah telah di laporkan ke owner DN7 BOT, Laporan palsu atau main² tidak akan ditanggapi.')
					break
		case 'request':
					...
					itsmeiky.sendMessage('6281219144171@s.whatsapp.net', options, text, {quoted: iky})
					itsmeiky.sendMessage('6281219144171@s.whatsapp.net', options, text, {quoted: iky})
					reply('REQUEST ANDA TELAH SAMPAI KE OWNER DN7 BOT, Requests palsu atau main² tidak akan ditanggapi.')
					break
		case 'blocklist': 
		...
		case 'bc': 
					...
					if (isMedia && !iky.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST DN7 BOT 」*\n\n${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
		...
		case 'kick':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (iky.message.extendedTextMessage === undefined || iky.message.extendedTextMessage === null) return reply('𝗧𝗮𝗴 𝘁𝗮𝗿𝗴𝗲𝘁 ??Y𝗮𝗻𝗴 𝗶𝗻𝗴𝗶𝗻 𝗱𝗶 𝘁𝗲𝗻𝗱𝗮𝗻𝗴!')
					mentioned = iky.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
